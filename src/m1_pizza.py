...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(300, 200),
                rg.Point(270, 270),
                rg.Point(200, 300),
                rg.Point(129.3, 270.7),
                rg.Point(100, 200),
                rg.Point(129.3, 129.3),
                rg.Point(200, 100),
                rg.Point(270.7, 129.3)]
...
def run_test_draw_points_on_circle():
    ...
    title = 'Test 4: 10 Points on Yellow Circle'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(300, 200), 75)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'Test 4'
    window = rg.RoseWindow(900, 900, title)
    circle = rg.Circle(rg.Point(500,500), 300)
    circle.fill_color = 'black'
    pizza(window, circle, 1800, 'red', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        slice = rg.Line(center, points[k])
        slice.thickness = thickness
        slice.color = color
        slice.attach_to(window)
    window.render()
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('Polygon Test 3: 4 segments with red lines')
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 2
    polygon(window, circle, 4, 'red', 10)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    line = rg.Line(points[len(points)-1], points[0])
    line.color = color
    line.thickness = thickness
    line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('Test 4, 100 segments with hops of 20')
    window = rg.RoseWindow(900, 900, title)
    circle = rg.Circle(rg.Point(450, 450), 200)
    circle.fill_color = 'orange'
    fancy_polygon(window, circle, 100, 20, 'yellow', 20)
    window.close_on_mouse_click()
